...
def get_utterances(input_csv, wav_filepaths):
	utterances = []
	with open(input_csv) as f:
		...
		for row in csv_reader:
			...
			voice_votes = row["VoiceVote"].split(":")
			voice_levels = list(map(lambda l: float(l), row["VoiceLevel"].split(":")))
...
def main():
	...
	utterances = get_utterances(os.path.join(input_data_dir, INPUT_CSV), wav_filepaths)
